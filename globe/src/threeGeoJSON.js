...
export function drawThreeGeo({ json, radius, materalOptions }) {
  ...
  function getColorFromEmission(emission) {
  const max = 20; // or your dataset’s max CO2
  const ratio = Math.min(emission / max, 1);
  ...
  function drawPolygon(x_values, y_values, z_values, color) {
  // Build shape in 2D projected space
  ...
  // Convert to geometry and map onto sphere
  ...
  // Reposition mesh so it wraps sphere
  // (we’ll reproject below, since shape is in 3D)
  const position = new THREE.BufferAttribute(new Float32Array(x_values.length * 3), 3);
  for (let i = 0; i < x_values.length; i++) {
    position.setXYZ(i, x_values[i], y_values[i], z_values[i]);
  }
